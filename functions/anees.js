...
    data = normalizeForUI(action, data);
    // حل مشكلة الرموز اللاتينية
    data = tidySymbolsForDisplay(data);
...
// إضافة علامة $ حول الرموز قبل عرضها في الجدول
function tidySymbolsForDisplay(data) {
  ...
  if (data.givens) data.givens = processArray(data.givens);
  if (data.unknowns) data.unknowns = processArray(data.unknowns);
  if (data.symbols) data.symbols = processArray(data.symbols);
  return data;
}
